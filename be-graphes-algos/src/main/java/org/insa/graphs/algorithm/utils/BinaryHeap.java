...
public class BinaryHeap<E extends Comparable<E>> implements PriorityQueue<E> {
    ...
    private int findIndexOf(E x, int index) {
        ...
        int ixRight = ixLeft+1;
        int foundIndex = findIndexOf(x, ixLeft);
        if (foundIndex != -1)
            return foundIndex;
        foundIndex = findIndexOf(x, ixRight);
        if (foundIndex != -1)
            return foundIndex;
